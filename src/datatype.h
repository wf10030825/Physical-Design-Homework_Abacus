...
// #include <thread>
// #include <atomic>
#include <omp.h> /// replace thread // ! �u��� index base
// #include <time.h>
#include <ctime>
#include <chrono> /// replace time.h


// Forward declaration for Tile (defined in corner_stitching.h)
struct Tile;

// Forward declaration for CornerStitching
class CornerStitching;

// Global CornerStitching pointer
extern CornerStitching* g_corner_stitching;
// ����׭p�� (ms)
...
#define AMP_PARA 1000.0 // ! NCKU : �Pchip�ۦP�j�p�����Шt (�{�bAMP_PARA�u�Φbscale��u��)
// #define AMP_PARA 1.0 // ! replace : ��chip�p�F1000�������Шt
...
#define INT_CONVERT(a) (int)(1.0 * (a) + 0.5f) /// �|�ˤ��J
...
// - Unit ���
extern prec unitResistanceName; // ! def lef �Ҭ� ohm

// - Layer
extern prec default_layer_UnitNomResistance; // ! def lef �Ҭ� ohm
...
// - VIA
extern prec default_VIA_UnitNomResistance; // ! def lef �Ҭ� ohm
extern prec default_VIA_Spacing;
extern FPOS default_VIA_W_H;

// - VIA unit
// ? �Pı�N�Orail layer �� VIA
// todo : 0711
extern prec default_via_RDL_UnitNomResistance; // ! def lef �Ҭ� ohm
// todo : 0326|0.0001
...
// - Constant
extern prec default_dbu_per_micron; // ! all def coor �n / dbu_per_micron , lef ����
...
extern string input_def_pl; /// ���wintput def or pl
// extern string DATE;
extern bool read_tf;  /// �_��case (PA_series)
extern bool read_aux; /// bookshelf // ! no FENCE or IR_drop
extern prec aux_scale;

// - GP mode
extern bool fence_mode; // ! �N��S�}�]��DEAFULT_FENCE
extern bool IR_mode;
...
extern prec target_density; // ! ���魱�n�Q�βv (filler�|���target_density)
...
// - Constant
extern prec dbu_per_micron; // ! all def coor �n / dbu_per_micron , lef ����

// - Chip info
extern FPOS chip_lb, chip_ur;
extern FPOS chip_W_H;
extern prec chip_area;
extern prec die_lb_x;
extern prec die_lb_y;
extern prec die_ur_x;
extern prec die_ur_y;
...
// - Power Mesh
extern VIA* via_RDL; // ? rail layer �� �Y��VIA
...
// #define ALPHAcGP 1e-12 // ? 1e-12 on paper but 1e-6 in implement // undo : �[��argument
...
public:
    ...
    // vector<FPOS> polys; /// Polygon
    vector<RECT> rects;

    // - Power
    prec static_power = 0, voltage = 0;
    CROSSNODE* crossnode = nullptr; /// only one crossnode
    ...
    // - Corner Stitching
    Tile* init_tile= nullptr; // initial copy
    Tile* work_tile = nullptr; // working copy
...
public:
    ...
    vector<PIN*> pins;
    vector<BLOCKAGE*> obstructions; /// default:MOS tripe | �h���:�h�ӯx��
    /// �h���
    ...
    string category = "";
    int orientation = 0;   /// 0(N), 1(W), 2(S), 3(E), 4(FN), 5(FW), 6(FS), 7(FE)
    bool is_cover = false; /// �T�w���i���| (terminal NI)
...
public:
    ...
    // string direction; /// routing layer default direction (VERTICAL / HORIZONTAL) (���@�w�u���@��V)
    int direction = -1; /// default -1 | 0:VERTICAL | 1:HORIZONTAL

    // - Power
    int metal_layer_id = -1; /// >= 0 �� metalLayer
...
public:
    ...
    string name = "";
    string lower_name = "", upper_name = "", cut_name = ""; // * parser �d���K�٬O�ݭn
    /// �b LEFViaCB, DEFViaCB ���u�|���s name
    /// ����b CheckVia �~�� *layer
    ...
    /// VIA �W�U���ݼh�����e
    /// upper_layer_left_right�G�W�hW
    /// upper_layer_up_low�G�W�hH
    /// lower_layer_left_right�G�U�hW
    /// lower_layer_up_low�G�U�hH
...
public:
    ...
    // double width, length;
    FPOS lb, ur;
    FPOS W_H; // prec W = 0, H = 0;  // ? SEGMENT �p��
...
public:
    ...
    //abacus
    vector<ROW*> sub_rows; // 新增：儲存切分後的子列
};
...
public:
    ...
    prec voltage = 0;
    prec pad_current = 0, static_current = 0; /// static_current = std_current + pad_currernt (�u�ݭn��sstd_current)
...
// * ��ı�o�i�H�W�ߥX�� �b��L������ƭp�⪺�a��b�ŧi PY

// ? ��ץ����W�ߥX�ӡA���n�g�b��Ƶ��c�̹�a? (PY)
// * Yeah (Easymoney)
